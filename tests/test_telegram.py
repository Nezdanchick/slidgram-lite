...
import slidgram
import slidgram.client
import slidgram.gateway
...
class TestTelegram(TestTelegramBase):
    def setUp(self):
        ...
        slidgram.session.TelegramClient = MockTdlib
        slidgram.gateway.Gateway.args = Namespace(tdlib_key="", tdlib_path="")
        super().setUp()
...
async def test_ignore_read_marks_confirmation():
    ...
    await slidgram.client.TelegramClient.handle_ChatReadInbox(tg, action)
